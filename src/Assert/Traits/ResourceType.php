...
trait ResourceType
{
	...
	{
		$message = $message ?: sprintf('Given value %s is not a resource', Variable::stringify($value));
		static::throwUnless(is_resource($value), $message);
	}

	public static function notResource(mixed $value, string $message = ''): void
	{
		$message = $message ?: sprintf('Given value %s is a resource', Variable::stringify($value));
