...
final class AssertTest extends TestCase
{
	...
	public function test_it_type_assertions_work_properly($function, $valid, $invalid, $message)
	{
		...
		foreach ($invalid as $item) {
			$this->throwsException(fn () => Assert::$function($item))
				...
				->withMessage(sprintf($message, Variable::stringify($item)))
				->validate();
		}
	}

	/** @dataProvider complexProvider */
	public function test_it_complex_assertions_work_properly($function, $valid, $invalid, $message)
	{
		foreach ($valid as $item) {
			Assert::$function(...$item);
		}
	...
	public function test_it_permission_assertions_work_properly($function, $valid, $invalid, $message)
	{
		...
		foreach ($invalid as $item) {
			$this->throwsException(fn () => $file->setPermission($item) && Assert::$function($file->getPath()))
				->withType(InvalidArgumentException::class)
				->withMessage(sprintf($message, Variable::stringify($file->getPath())))
				->validate();
		}
	...
	public function test_it_symlink_assertions_work_properly()
	{
	...
	public function test_it_upload_file_assertions_work_properly()
	{
		$file = $this->makeTemporaryFile();
